...
Server routes can use dynamic parameters within brackets in the file name like `/api/hello/[name].ts` and be accessed via `event.context.params` using [`getRouterParam`](https://h3.unjs.io/utils/request#getrouterparamevent-name-opts-decode) or [`getRouterParams`](https://h3.unjs.io/utils/request#getrouterparamsevent-opts-decode):
